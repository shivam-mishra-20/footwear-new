...
function Inventory() {
  ...
  const [editVariantSize, setEditVariantSize] = useState(null);
  ...
  const handleAddProduct = async (e) => {
    ...
    setSuccess("");
    try {
      const docId = barcode; // Use barcode as main product ID
      ...
      if (editId && editVariantSize) {
        // Editing existing variant
        const productRef = doc(db, "ProductsRegistered", editId);
        ...
        if (productSnap.exists()) {
          ...
          setSuccess(`Size ${size} updated successfully!`);
        }
      } else if (existing) {
        ...
        await updateDoc(doc(db, "ProductsRegistered", existing.id), {
          variants,
          name,
          description,
          gender,
        ...
        setSuccess("Product with size variant created successfully!");
      }
      setBarcode("");
      setName("");
      setDescription("");
      ...
      setError("Failed to register/update product: " + err.message);
    }
    setLoading(false);
  };

  const handleEdit = (product, variantSize = null) => {
    setEditId(product.id);
    setEditVariantSize(variantSize);
    ...
    if (variantSize && product.variants && product.variants[variantSize]) {
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filteredProducts.map((product) => {
                        ...
                        const totalStock = variantKeys.reduce(
                          (sum, key) => sum + Number(variants[key]?.stock || 0),
                          0
                        );
                            ...
                            {isExpanded &&
                              variantKeys.map((sizeKey) => {
                                ...
                                return (
                                  ...
                                  >
                                    <td className="px-6 py-3 pl-12 text-sm text-gray-500">
                                      —
                                    </td>
